...
SVSM=""
CONSOLE="serial"
UEFI_BIOS_CODE="./usr/local/share/qemu/OVMF.fd"
...
usage() {
	...
	echo " -measured     measured boot with kernel, initrd, and command-line arguments"
    echo " -idblock      identity and author blocks"
	exit 1
}
...
if [ -n "${SEV_GUEST}" ]; then
	...
	if [ -n "$SEV_SNP_GUEST" ]; then
		...
		add_opts "-object sev-snp-guest,auth-key-enabled=on,$SEV_COMMON,init-flags=${SNP_FLAGS},host-data=b2l3bmNvd3FuY21wbXA${DISCARD:+,discard=$DISCARD},kernel-hashes=${KERNEL_HASHES},reduced-phys-bits=1,${ID_BLOCK}"
	else
